...
import type { InspectionView, RowState } from "@/lib/useInspection";
...
export function HomeScreen({ view, onStart, onStartOver }: HomeScreenProps) {
  return (
    <>
      <div className="flex-1 overflow-y-auto px-5 pb-[120px]">
        ...
        <p className="mt-[22px] text-xs leading-[1.5] text-steel-600">
          Questions?{" "}
          <a className="text-aqua-700 hover:text-accent" href={`mailto:${SUPPORT_EMAIL}`}>
            {SUPPORT_EMAIL}
          </a>{" "}
          · {SUPPORT_PHONE}
        </p>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-bg from-72% to-transparent px-5 pt-4 pb-[26px]">
        <Button
          variant="primary"
          block
          onClick={onStart}
